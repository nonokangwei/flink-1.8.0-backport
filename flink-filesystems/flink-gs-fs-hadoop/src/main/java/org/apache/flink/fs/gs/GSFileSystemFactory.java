...
import org.apache.flink.shaded.util.Preconditions;
...
public class GSFileSystemFactory implements FileSystemFactory {
    ...
    public void configure(Configuration flinkConfig) {
        ...
        org.apache.flink.shaded.configuration.Configuration shadedConfiguration =  org.apache.flink.shaded.configuration.Configuration.fromMap(flinkConfig.toMap());

        // load Hadoop config
        this.hadoopConfig = ConfigUtils.getHadoopConfiguration(shadedConfiguration, configContext);
        LOGGER.info(
                "Using Hadoop configuration {}", ConfigUtils.stringifyHadoopConfig(hadoopConfig));

        // construct file-system options
        this.fileSystemOptions = new GSFileSystemOptions(shadedConfiguration);
